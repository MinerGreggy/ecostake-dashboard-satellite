...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'ecostake-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('ecostake').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('ecostake')[service]);
